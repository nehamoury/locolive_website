...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onSignup }) => {
  return (
    <div className="min-h-screen bg-[#0a0a0c] text-white font-sans relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-[-200px] left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-violet-600/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 md:px-16 py-5 border-b border-white/5 backdrop-blur-sm">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xl font-black tracking-tight">Locolive</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={onSignup}
            className="px-5 py-2 text-sm font-bold bg-violet-600 hover:bg-violet-500 text-white rounded-full transition-all active:scale-95"
          >
            Sign Up
          </button>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center text-center px-6 pt-20 pb-16">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-tight tracking-tight mb-6 max-w-3xl">
            Discover What&apos;s{' '}
          ...
          <p className="text-gray-400 max-w-xl mx-auto leading-relaxed text-base md:text-lg mb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={onSignup}
              className="flex items-center justify-center gap-2 px-7 py-3.5 bg-gradient-to-r from-violet-600 to-pink-600 text-white font-bold rounded-full shadow-xl shadow-violet-500/25 hover:opacity-90 transition-all active:scale-95"
            >
              Get Started Free <ArrowRight className="w-4 h-4" />
            </button>
            <button
              onClick={onLogin}
              className="flex items-center justify-center gap-2 px-7 py-3.5 bg-white/5 text-white font-bold rounded-full border border-white/10 hover:bg-white/10 transition-all active:scale-95"
            >
              Explore Map
            </button>
          </div>
        ...
        >
          {features.map((f) => (
            <div
              key={f.title}
              className="flex flex-col gap-2 p-5 rounded-2xl bg-white/[0.04] border border-white/[0.07] hover:bg-white/[0.07] transition-all text-left"
            >
              {f.icon}
              <p className="font-bold text-sm mt-1">{f.title}</p>
      ...
      {/* Footer */}
      <footer className="relative z-10 text-center py-8 text-xs text-gray-600 border-t border-white/5">
