      ...
      default:
        return (
          <button
            ...
            className="px-3 py-1.5 bg-primary text-white rounded-lg text-xs font-bold flex items-center gap-1.5 hover:opacity-90 transition-colors shrink-0 active:scale-95 shadow-md shadow-primary/20"
          >
    ...
    <div className="h-full bg-[#f9e8ff] text-black overflow-y-auto no-scrollbar pb-24 md:pb-0">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-black">Path Crossings</h1>
            <p className="text-xs text-black/60">People whose paths crossed yours recently</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center opacity-40">
            <Footprints className="w-14 h-14 mb-4 text-black/40" />
            <p className="text-sm font-medium max-w-[250px] text-black/60">No path crossings yet. Walk around your area and discover people nearby!</p>
          </div>
        ) : (
          <div className="space-y-3">
            {crossings.map((crossing, idx) => (
              <div key={idx} className="flex items-center p-4 bg-primary/5 rounded-2xl border border-primary/10 hover:bg-primary/10 transition-all">
                <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-primary to-accent flex items-center justify-center font-bold text-sm mr-4 shrink-0 text-white">
                  {crossing.username?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm text-black">@{crossing.username}</p>
                  <div className="flex items-center space-x-3 mt-1">
                    <span className="flex items-center space-x-1 text-[10px] text-black/40">
