...
const CrossingsView: FC<CrossingsViewProps> = ({ onUserSelect }) => {
  ...
  const getButtonContent = (userId: string) => {
    ...
    switch (status) {
      case 'sending':
        return (
          <div className="w-8 h-8 rounded-full bg-bg-sidebar flex items-center justify-center">
      ...
      default:
        return (
          <button
            ...
            className="w-10 h-10 rounded-full bg-gradient-to-tr from-primary to-accent flex items-center justify-center text-white shadow-lg shadow-primary/20 transition-all hover:scale-110 active:scale-95 cursor-pointer"
          >
  ...
  return (
    <div className="h-full bg-bg-base text-text-base overflow-y-auto no-scrollbar pb-32 transition-colors duration-300">
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="flex items-end justify-between mb-12">
            <div>
                ...
                >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[3px] text-primary">Real Discovery</span>
                </motion.div>
                <h1 className="text-4xl font-black italic uppercase tracking-tighter leading-none">Path Crossings</h1>
                <p className="text-xs font-bold text-text-muted mt-2">Discover who you've met in the real world.</p>
            </div>
        </div>
        ...
        {loading ? (
             <div className="space-y-4">
                {[1,2,3].map(i => (
                    <div key={i} className="h-24 bg-bg-sidebar rounded-[32px] animate-pulse" />
                ))}
             </div>
        ) : crossings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-24 h-24 bg-bg-sidebar rounded-[40px] flex items-center justify-center mb-6">
